import { expect } from "chai";

import {
  getBrowserPageview,
  areDifferent,
  camelize,
  getPlaybackPercentage
} from "./utilities";

describe("(Utils) utilities", () => {
  context("getBrowserPageview", () => {
    it("should return the current pathname", () => {
      const { page } = getBrowserPageview();
      expect(page).equals(
        document.location.pathname + document.location.search
      );
    });

    it("should return the current title", () => {
      const { title } = getBrowserPageview();
      expect(title).equals(document.title);
    });
  });

  context("areDifferent", () => {
    it("should return false for same objects", () => {
      expect(areDifferent({ foo: "bar" }, { foo: "bar" })).equals(false);
    });

    it("should return true for name differences", () => {
      expect(areDifferent({ foo: "bar" }, { foo: "bar", bar: "baz" })).equals(
        true
      );
    });

    it("should return true for value differences", () => {
      expect(areDifferent({ foo: "bar" }, { foo: "baz" })).equals(true);
    });
  });

  context("camelize", () => {
    it("should return a camelized string", () => {
      expect(camelize("i-should-be-camelized")).equals("iShouldBeCamelized");
    });

    it("should return the original value without trying to camelize it", () => {
      expect(camelize(123)).equals(123);
    });
  });

  context("getPlaybackPercentage", () => {
    it("should return a correct percentage", () => {
      expect(getPlaybackPercentage(30, 100)).equals(30);
      expect(getPlaybackPercentage(30, 200)).equals(15);
      expect(getPlaybackPercentage(87, 162)).equals(53);
    });
  });
});
